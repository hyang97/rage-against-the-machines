...
void Player::moveOrAttack(int dir)
{
    ...
    if(m_arena->determineNewPosition(newRow, newCol, dir)){
        
        //move is legal
        if(m_arena->nRobotsAt(newRow, newCol) > 0){
            //facing robot, attack and don't move
            m_arena->attackRobotAt(newRow, newCol, dir);
        }
        else{
            //complete the move!
            m_row = newRow;
...
bool Arena::determineNewPosition(int& r, int& c, int dir) const
{
    ...
    if(newRow < 1 || newCol < 1 || newRow > m_rows || newCol > m_cols){
        //illegal move
        return false;
    }
    else{
